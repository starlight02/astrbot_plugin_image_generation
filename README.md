...
- 预设系统：支持简单提示词预设，也支持带比例、分辨率的高级预设。
...
| 命令                                    | 说明                                                                |
| :-------------------------------------- | :------------------------------------------------------------------ |
| `/生图 <提示词或预设名称> [额外提示词]` | 生成图片。消息、引用消息或 @ 用户中包含图片时会自动进入图生图模式。 |
| `/生图模型`                             | 查看可用模型和当前模型。                                            |
| `/生图模型 <序号>`                      | 切换到指定序号的模型。                                              |
| `/预设`                                 | 查看所有预设。                                                      |
| `/预设 添加 <预设名:预设内容>`          | 添加预设。                                                          |
...
| 配置名        | 默认值 | 说明                                |
| :------------ | :----- | :---------------------------------- |
| 启用 LLM 工具 | 开启   | 允许 LLM 在对话中自动调用生图功能。 |
...
开始生图任务提示模板支持这些占位符：`{reference_image_count}`、`{prompt}`、`{preset}`、`{aspect_ratio}`、`{resolution}`、`{task_id}`、`{model}`、`{mode}`、`{reference_images_block}`、`{preset_block}`。
