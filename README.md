...
| 显示结果信息         | 用量                 | 发送图片后可附加耗时、模型、生成数量、用量；用量仅在开启每日额度时显示。                        |
...
`/预设` 会同时展示预设和人设。预设与人设同名时，`/生图` 优先使用预设。LLM 生图工具可通过 `preset` 参数使用预设，也可通过 `persona` 参数使用人设；当填写人设且未指定比例时默认使用 `9:16`，并支持 `prompt`、`aspect_ratio`、`resolution`、`avatar_references` 和 `reference_images` 参数。LLM 生图工具只使用显式传入的参考图和人设参考图，不会自动读取当前消息图片、引用图片或 @ 头像，并会在提交任务前去除重复参考图。LLM 预设查询工具默认只返回预设或人设名称列表，也可以通过名称查看具体内容。LLM 预设编辑工具可创建和删除预设，创建时支持简单提示词格式和高级 JSON 格式。
